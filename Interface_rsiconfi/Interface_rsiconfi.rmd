...
execute_SQL <- function(a_demo, a_anexo, rotulo_eixo_x, rotulo_eixo_y, opcao, filtro_opcional=""){
  ...
  user<- "USR_GTCEAD" 
  senha<-"67285133" 
  con1 <- dbConnect(drv, user , senha, "STN_DATALAKE.WORLD" )
  
#       consulta <- dbSendQuery(con1, 
#           "
#           select * 
#           from PRD_SICONFI.XBRL_FATONUMERICO 
#           where
# CO_TIPO_DEMONSTRATIVO='QDCC' and 
# NO_ANEXO = 'DCA-Anexo I-E' and
# AN_EXERCICIO in (2017,2018) and
# sg_ENTE in ('SP', 'MG','BA') and
# NO_LABEL_EIXO_Y in('01 - Legislativa', '10 - Saúde', '12 - Educação') and
# NO_LABEL_EIXO_X	in('Despesas Liquidadas')
          # 
          # 
          # 
          # ")
  ...
  cons_colunas<- "AN_EXERCICIO, CO_ESFERA, CO_PODER, ID_ENTE, NO_ENTE, SG_ENTE, NO_LABEL_EIXO_X, NO_LABEL_EIXO_Y, VALUE"
  
  cons_tabela<- "PRD_SICONFI.XBRL_FATONUMERICO"
  ...
  df_consulta<- map_dfr(0:rept, function(arg_i){
    ...
    filtro_obrigatorio<-
      paste0( 
        ...
        lista_ano,
        ") and ",
        ifelse(opcao_uf,"CO_ESFERA = 'M' and SG_ENTE in(","ID_ENTE in(" ),
...
#dbDisconnect(con)
  ...
  names(df_consulta)[7:8]<- c(rotulo_eixo_x, rotulo_eixo_y)
...
generate_dt <- function(opcao){
  ...
  if (opcao$topico ==1){
    ...
    filtro_complementar<- paste0(" NO_LABEL_EIXO_Y in(","'Total Receitas'",")")
  ...
  #    lista_instituicao_script<- paste0("'",lista_instituicao[1], "'")
  # for (instituicao in lista_instituicao[-1]){
  #   lista_instituicao_script<- paste0(lista_ano,",","'", ano, "'")
  # }
